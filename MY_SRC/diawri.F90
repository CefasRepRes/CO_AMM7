...
CONTAINS
   ...
   SUBROUTINE dia_wri( kt )
      ...
      IF ( iom_use("davmdz") ) THEN
         z3d(:,:,jpk) = 0._wp
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi   ! vector opt.
                  z3d(ji,jj,jk) = ( avm(ji,jj,jk+1) - avm(ji,jj,jk) ) / &
                                  ( gdepw_n(ji,jj,jk+1) - gdepw_n(ji,jj,jk) ) * wmask(ji,jj,jk)

               END DO
            END DO
         END DO
         CALL iom_put( "davmdz" ,  z3d )          
      ENDIF

      IF ( iom_use("avmgrad") ) THEN
         z3d(:,:,jpk) = 0._wp
         DO jj = 1, jpj
            DO ji = 1, jpi   ! vector opt.
               z3d(ji,jj,1) = ( avm(ji,jj,2) - avm(ji,jj,1) ) / &
                                  ( gdepw_n(ji,jj,2) - gdepw_n(ji,jj,1) ) * wmask(ji,jj,1)
               DO jk = 2, jpkm1
                  zztmp = ( avm(ji,jj,jk) - avm(ji,jj,jk-1) ) / &
                          ( gdepw_n(ji,jj,jk) - gdepw_n(ji,jj,jk-1) ) * wmask(ji,jj,jk-1)
